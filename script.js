...
function applyCodesFilter() {
    ...
    else if (filter === 'trial') filtered = codesData.filter(c => c.duration_label && c.duration_label.includes('3 ساعات'));
...
function displayCodes(codes) {
    ...
    tbody.innerHTML = codes.map((c, i) => {
        ...
        const isTrial    = c.duration_label && c.duration_label.includes('3 ساعات');
        ...
        if (isTrial) {
            ...
            const diffDays = Math.round((expires - created) / 86400000);
            let label;
            if (diffDays === 1)       label = '🕐 24 ساعة';
            else if (diffDays === 7)  label = '📅 أسبوع';
            else if (diffDays === 30) label = '📆 شهر';
            else if (diffDays === 90) label = '📆 3 شهور';
            else                      label = `📆 ${diffDays} يوم`;
...
function updateSummary() {
    ...
    let dur = type === 'lifetime' ? '♾️ مدى الحياة'
            : type === 'preset' && generatorState.days === 0.125 ? '⚡ 3 ساعات تجربة'
...
async function generateCodes() {
    ...
    try {
        const rows = [];
        const isTrial = generatorState.type === 'preset' && generatorState.days === 0.125;
        for (let i = 0; i < count; i++) {
            ...
            if (generatorState.type === 'lifetime') {
                ...
                // 3 ساعات = 3 * 3600 * 1000 ms
                expiresAt = new Date(Date.now() + 3 * 3600000).toISOString();
            } else if (generatorState.type === 'date' && generatorState.expiryDate) {
                expiresAt = new Date(generatorState.expiryDate).toISOString();
            } else {
                expiresAt = new Date(Date.now() + (generatorState.days||30)*86400000).toISOString();
            }
            const durationLabel = generatorState.type === 'lifetime' ? '♾️ مدى الحياة'
                : isTrial ? '⚡ 3 ساعات تجربة'
                : generatorState.type === 'date' ? `حتى ${generatorState.expiryDate}`
                : generatorState.days === 1 ? '🕐 24 ساعة'
                : generatorState.days === 7 ? '📅 7 أيام'
                : generatorState.days === 30 ? '📆 30 يوم'
                : generatorState.days === 90 ? '📆 3 شهور'
                : `📆 ${generatorState.days} يوم`;
            rows.push({ license_key: key, expires_at: expiresAt, status: 'active', duration_label: durationLabel });
        }
...
document.getElementById('saveSettingsBtn')?.addEventListener('click', async () => {
    ...
    const trial_hours = parseInt(document.getElementById('trialHours')?.value) || 24;
    await sb.from('app_settings').upsert({ id:1, is_shutdown, bot_mode, global_message, trial_hours });
