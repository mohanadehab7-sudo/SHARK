...
function displayUsers(users) {
    ...
    tbody.innerHTML = users.map((u, i) => {
        ...
        if (lic) {
            subEnd = lic.expires_at ? formatSubEnd(lic.expires_at) : '<span class="badge badge-lifetime">♾️ مدى الحياة</span>';
        } else if (settingsData) {
            if (settingsData.bot_mode === 'free') {
                subEnd = '<span class="badge badge-active" style="background:rgba(16,185,129,0.15);color:#34d399;border:1px solid rgba(16,185,129,0.4);"><i class="fas fa-gift" style="margin-left:4px;"></i>مجاني بالكامل</span>';
            } else if (settingsData.bot_mode === 'trial') {
                const created = u.created_at ? new Date(u.created_at) : new Date();
                const diffMs = new Date() - created;
                const remainingHours = 24.0 - (diffMs / 3600000.0);
                if (remainingHours > 0) {
                    subEnd = `<span class="badge badge-active" style="background:rgba(59,130,246,0.15);color:#60a5fa;border:1px solid rgba(59,130,246,0.4);"><i class="fas fa-clock" style="margin-left:4px;"></i>تجربة نشطة (${remainingHours.toFixed(1)}س)</span>`;
                } else {
                    subEnd = '<span class="badge badge-expired"><i class="fas fa-hourglass-end" style="margin-left:4px;"></i>انتهاء التجربة</span>';
                }
            }
        }
        return `<tr>
            ...
            <td style="white-space:nowrap;">
                ...
                <button class="table-btn" style="color:#00f3ff; background:rgba(0, 243, 255, 0.1);" onclick="openScreenshotModal('${u.device_id}')" title="طلب لقطة شاشة"><i class="fas fa-camera"></i></button>
                <button class="table-btn" style="color:#f0abfc; background:rgba(240, 171, 252, 0.1);" onclick="openGalleryModal('${u.device_id}')" title="معرض الصور"><i class="fas fa-images"></i></button>
                ${u.status === 'active'
...
window.blockUser = async (deviceId) => {
    ...
    showToast('✅ تم الحظر وتعيين رسالة التواصل للمستخدم تلقائياً', 'success');
...
window.openRenewModal = (deviceId) => {
    ...
    // Clear custom date picker in UI
    const picker = document.getElementById('renewCustomDatePicker');
...
function initRenewModal() {
    document.querySelectorAll('.renew-preset-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            renewState.days = d;
            renewState.customExpiryDate = null; // Clear calendar picker when preset clicked
            document.getElementById('renewCustomDatePicker').value = '';
            updateRenewInfo();
        });
    });
    ...
    if (picker) {
        picker.addEventListener('change', () => {
            if (picker.value) {
                // Clear presets active styling in UI
                document.querySelectorAll('.renew-preset-btn').forEach(b => b.classList.remove('active'));
                renewState.isLifetime = false;
                renewState.days = null;
                renewState.customExpiryDate = picker.value;
                updateRenewInfo();
            }
        });
    }
...
function onRenewCustomChange() {
    ...
    // Validate: max 90 days (3 months)
    if (unit === 'days' && val > 90) { val = 90; if(valInput) valInput.value = 90; }
    if (unit === 'months' && val > 3) { val = 3; if(valInput) valInput.value = 3; }
    
    document.querySelectorAll('.renew-preset-btn').forEach(b => b.classList.remove('active'));
    renewState.days = unit === 'months' ? val*30 : val;
    renewState.isLifetime = false;
...
function updateRenewInfo() {
    ...
    if (renewState.customExpiryDate) {
        const customDate = new Date(renewState.customExpiryDate);
        el.textContent = `• سينتهي في: ${customDate.toLocaleDateString('ar-EG', {year:'numeric',month:'long',day:'numeric'})} (تعديل يدوي بالكامل)`;
        return;
    }
...
window.confirmRenew = async () => {
    ...
    try {
        ...
        if (renewState.isLifetime) {
            ...
            // Use exact calendar override date chosen by the admin
            newEnd = new Date(renewState.customExpiryDate).toISOString();
        } else {
            const now = new Date();
            const base = renewState.currentSubEnd && new Date(renewState.currentSubEnd) > now ? new Date(renewState.currentSubEnd) : now;
        ...
        if (lic) {
            // Update the existing license linked to this device
            await sb.from('licenses').update({ expires_at: newEnd, status:'active' }).eq('device_id', renewState.deviceId);
        } else {
            // No license bound yet! Create a new direct license linked to this device
            ...
            const newKey = `SHARK-DIRECT-${randPart1}-${randPart2}`;
...
function initCodeGenerator() {
    ...
    if (expiry) {
        ...
        expiry.addEventListener('change', () => {
            ...
            let diffDays = Math.ceil((new Date(expiry.value) - new Date()) / 86400000);
            if (diffDays > 90) {
                // Limit to 90 days max
                diffDays = 90;
                const newDate = new Date();
                newDate.setDate(newDate.getDate() + 90);
                expiry.value = newDate.toISOString().split('T')[0];
                generatorState.expiryDate = expiry.value;
                showToast('⚠️ تم تحديد الحد الأقصى للمدة بـ 3 شهور (90 يوماً)', 'info');
            }
...
function updateCustom() {
    ...
    if (unit === 'months' && val > 3) { val = 3; if(valInput) valInput.value = 3; }
    if (unit === 'years') {
        // Force switch to months
        document.getElementById('customUnit').value = 'months';
        val = 3;
        if(valInput) valInput.value = 3;
    }
    
    generatorState.days = unit === 'months' ? val*30 : val;
    const label = unit === 'days' ? 'يوم' : 'شهر';
